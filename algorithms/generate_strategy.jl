...
"""
    generate_strategy_from_average_order(game::Vector{SGNode}, average_node_order::Vector{Int}, max_tails::Dict{Int, Vector{Tuple{Int, Bool}}}, min_parent_map::Dict{Int, Vector{Int}}, labeled::Vector{Int}, queue::Vector{Int})

Create a strategy from an ordering of average nodes
...
- `queue::Vector{Int}`: the process queue
"""
function generate_strategy_from_average_order(game::Vector{SGNode}, average_node_order::Vector{Int}, max_tails::Dict{Int, Vector{Tuple{Int, Bool}}}, min_parent_map::Dict{Int, Vector{Int}}, labeled::Vector{Int}, queue::Vector{Int})
    labeled .= 0
    empty!(queue)
    ...
    @inbounds for (id,node) in enumerate(game)
        if node.type == minimizer || node.type == maximizer
            if labeled[id] == 1
                strat[id] = node.arc_a
            elseif labeled[id] == 2
                strat[id] = node.arc_b
