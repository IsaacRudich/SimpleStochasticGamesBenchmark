...
function get_longest_acyclic_path_fast(game::Vector{SGNode}, node_index::Int)
    ...
    while !isempty(queue)
        ...
        @inbounds for node in queue
            longest_path = get_longest_acyclic_path_fast_subroutine!(node, game[node.id].arc_a, game, new_queue, longest_path)
            longest_path = get_longest_acyclic_path_fast_subroutine!(node, game[node.id].arc_b, game, new_queue, longest_path)
        end
        queue = copy(new_queue)
    end
...
"""
    get_longest_acyclic_path_subroutine!(node::PathNode_Light, child::Int, game::Vector{SGNode}, queue::Set{PathNode_Light}, longest_path::Int)
...
"""
function get_longest_acyclic_path_fast_subroutine!(node::PathNode_Light, child::Int, game::Vector{SGNode}, queue::Set{PathNode_Light}, longest_path::Int)
    if !node.visited[child]  #no cycles
        #create the new child
        ...
        if game[child].type == terminal1 || game[child].type == terminal0
            #if the new node has a longer longest path, update longest path
            if sum(new_node.visited) > longest_path
                longest_path = sum(new_node.visited)
            end
        else #the new node is not a terminal keep searching
            push!(queue, new_node)
        end
...
"""
    get_longest_acyclic_paths_to_max_nodes_fast(game::Vector{SGNode})
...
- `game::Vector{SGNode}`: The SSG
"""
function get_longest_acyclic_paths_to_max_nodes_fast(game::Vector{SGNode})
    longest_path_values = Dict{Int, Int}()

    for (i, node) in enumerate(game)
        if node.type == maximizer
            println("workin on $i")
            longest_path_values[i] = get_longest_acyclic_path_fast(game, i)
        end
    end
