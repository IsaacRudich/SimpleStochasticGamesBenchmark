...
function graph_analysis_data()
    analysis = read_balanced_analysis_file("balanced_4096.txt")

    # Extract hk_avg for each problem type into an array of arrays
    hk_avg_values = Float64[]
    mod_hk_avg_values = Float64[]
    problem_type_labels = String[]
    problem_types = collect(keys(analysis))
    sort!(problem_types)
    ...
    for ptype in problem_types
        ...
        for dp in analysis[ptype]
            push!(hk_avg_values, dp.hk_avg)
            push!(mod_hk_avg_values, dp.mod_hk_avg)
            push!(problem_type_labels, ptype)
        end
    end

    x = [problem_type_labels; problem_type_labels]
    y = [hk_avg_values; mod_hk_avg_values]
    g = [fill("hk", length(hk_avg_values)); fill("mod-hk", length(hk_avg_values))]
    println(typeof(x), " ", typeof(y), " ", typeof(g))
    plot = groupedboxplot(x ,y, group = g, bar_width = 0.8)

    display(plot)

    savefig(plot, "./graphs/iterations")
    ...
    for ptype in problem_types
        # Append hk_avg values and their corresponding problem type label to the arrays
        for dp in analysis[ptype]
            push!(hk_avg_values, dp.hk_avg_time)
            push!(mod_hk_avg_values, dp.mod_hk_avg_time)
            push!(problem_type_labels, ptype)
        end
    end
